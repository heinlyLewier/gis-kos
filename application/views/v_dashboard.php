            ...
            <?php foreach ($list->result() as $key => $value) : ?>
                L.marker([<?= $value->latitude; ?>, <?= $value->longitude; ?>]).addTo(map)
                    .bindPopup(
                        ...
                        location.coords.latitude + "," + location.coords.longitude + "&destination=<?= $value->latitude ?>,<?= $value->longitude ?>' class='btn btn-sm btn-success text-light border-0 mt-4 mx-0' target='_blank' style='background-color:#298505'><i class= 'fas fa-route' ></i> Rute</a>"
                    );
            <?php endforeach; ?>
        });
    <?php } else { ?>
